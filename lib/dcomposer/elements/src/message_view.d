...
class MESSAGE_VIEW :ELEMENT
{
    ...
    void Engage()
    {
        foreach(OpenDocIF; DocMan.GetOpenDocs())
        {
            auto OpenDoc = cast(DOCUMENT)OpenDocIF;

            auto tag = OpenDoc.getBuffer.createTag("ErrorUnderLine", "underline", PangoUnderline.ERROR);
            if(tag is null) ShowMessage("crap", "error creating an error tag:)", "OK");
        }
    ...
    string Name (){return "Message Viewer";}
