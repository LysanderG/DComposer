...
class DIR_VIEW : ELEMENT
{
    ...
    void Refresh()
    {
        ...
        foreach(DirEntry entry; dirEntries(mCurrentFolder, filterText, SpanMode.shallow))
        {
            ...
            string stringsize;
            if(filesize > 1_024) stringsize = format("% 6sK", filesize/1024);
            else stringsize = format("% 6sB", filesize);
            if(filesize > 1_048_576)stringsize = format("% 6sM", filesize/1_048_576);
    ...
    void Engage()
    {
        ...
        mMaximumFilters = Config.GetValue("dir_view", "max_filters", 10);
    ...
    void Disengage()
    {
        ...
        if(filters2save.length > mMaximumFilters) filters2save.length = mMaximumFilters;
